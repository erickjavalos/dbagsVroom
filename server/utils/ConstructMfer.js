...
class ConstructMfer {
    ...
    generateDbagAuto = async (dbagImage, auto) => {
        ...
        for (let i = 0; i < this.AUTO_LAYERING_ORDER.length; i++) {
            ...
            if (layers[layer] !== undefined && layers[layer] !== null) {
                // append asset
                assetFileLocations.push(`./assets/auto_assets/${layer}/${layers[layer]}.png`)
            }
        }
    ...
    generateDbag = async (mfer) => {
        ...
        for (let i = 0; i < this.DBAG_LAYERING_ORDER.length; i++) {
            ...
            if (layer === "BodyType" && layers[layer] === "") {
                ...
                if (layers[layer] === "Captain's hat") {
                    assetFileLocations.push(`./assets/mfers_assets/${layer}/Captain_s hat.png`)
                }
                else {
                    assetFileLocations.push(`./assets/mfers_assets/${layer}/${layers[layer]}.png`)
                }
            }
        }
        // read in image using jimp
